...
export const generateQRCodeData = (certificateData: CertificateData): string => {
  const qrData = {
...
export const validateCertificateIntegrity = (data: CertificateData): {
  isValid: boolean;
  errors: string[];
} => {
  const errors: string[] = [];
  ...
  if (!data.certificateTitle?.trim()) errors.push('Certificate title is missing');
  ...
  if (!data.course?.credentialId?.trim()) errors.push('Credential ID is missing');
  if (!data.course?.instructors?.length || !data.course.instructors[0]?.name?.trim()) {
    errors.push('At least one instructor is required');
  }
  if (!data.institution?.name?.trim()) errors.push('Institution name is missing');
